...
export class PiWebviewPanel {
  ...
  async show(): Promise<void> {
    ...
    // If PiService was already initialized (e.g. by sidebar), replay messages
    if (this.piService.initialized) {
      this.replayMessages();
    }
  }
  ...
  private replayMessages(): void {
    ...
    var entries = this.piService.sessionManagerInstance?.getEntries?.() ?? [];
    ...
    void this.piService.sendInitialMessages(pm, from, initialLimit).then(function () {
